...
class test_cbed_multi_slice_single_beam(chex.TestCase):
    ...
    def test_cbed_multi_slice_shape_and_type(
        self, shape, slice_thickness, voltage_kV, calib_ang
    ):
        # Generate random complex input data
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape, dtype=jnp.complex64)
        beam = jax.random.normal(key, shape[:2], dtype=jnp.complex64)

        # Get the variant of the function to test
        variant_cbed_multi_slice_single_beam = self.variant(
            cbed_multi_slice_single_beam
        )

        # Run the function
        result = variant_cbed_multi_slice_single_beam(
            pot_slice, beam, slice_thickness, voltage_kV, calib_ang
        )

        # Check the shape of the output
        chex.assert_shape(result, shape[:2])

        # Check the type of the output
        chex.assert_type(result, Float[Array, "H W"])
    ...
    def test_cbed_multi_slice_values(
        self, shape, slice_thickness, voltage_kV, calib_ang
    ):
        # Generate simple input data
        pot_slice = jnp.ones(shape, dtype=jnp.complex64)
        beam = jnp.ones(shape[:2], dtype=jnp.complex64)
        ...
        self.assertFalse(jnp.all(result == 0))
    ...
    def test_cbed_multi_slice_jit(self, shape, slice_thickness, voltage_kV, calib_ang):
        # Generate random complex input data
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape, dtype=jnp.complex64)
        beam = jax.random.normal(key, shape[:2], dtype=jnp.complex64)
        ...
        jitted_cbed_multi_slice = jax.jit(variant_cbed_multi_slice_single_beam)

        # Run the function
        result = jitted_cbed_multi_slice(
            pot_slice, beam, slice_thickness, voltage_kV, calib_ang
        )
...
class test_cbed_single_slice_single_beam(chex.TestCase):
    ...
    def test_cbed_single_slice_single_beam_shape_and_type(self, shape):
        # Generate random complex input data
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape, dtype=jnp.complex64)
        beam = jax.random.normal(key, shape, dtype=jnp.complex64)

        # Get the variant of the function to test
        variant_cbed_single_slice_single_beam = self.variant(
            cbed_single_slice_single_beam
        )

        # Run the function
        result = variant_cbed_single_slice_single_beam(pot_slice, beam)
    ...
    def test_cbed_single_slice_single_beam_values(self, shape):
        # Generate simple input data
        pot_slice = jnp.ones(shape, dtype=jnp.complex64)
        beam = jnp.ones(shape, dtype=jnp.complex64)
    ...
    def test_cbed_single_slice_single_beam_deterministic(self, shape):
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape, dtype=jnp.complex64)
        beam = jax.random.normal(key, shape, dtype=jnp.complex64)
        ...
        result1 = variant_cbed_single_slice_single_beam(pot_slice, beam)
        result2 = variant_cbed_single_slice_single_beam(pot_slice, beam)
...
class test_cbed_single_slice_multi_beam(chex.TestCase):
    ...
    @parameterized.parameters(
        ...
        {"shape": (20, 20), "num_modes": 3},
        {"shape": (30, 30), "num_modes": 5},
        {"shape": (50, 50), "num_modes": 10},
    )
    def test_cbed_single_slice_multi_beam_shape_and_type(self, shape, num_modes):
        # Generate random complex input data
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape, dtype=jnp.complex64)
        beam = jax.random.normal(key, shape + (num_modes,), dtype=jnp.complex64)

        # Get the variant of the function to test
        variant_cbed_single_slice_multi_beam = self.variant(
            cbed_single_slice_multi_beam
        )

        # Run the function
        result = variant_cbed_single_slice_multi_beam(pot_slice, beam)
    ...
    def test_cbed_single_slice_multi_beam_values(self, shape, num_modes):
        # Generate simple input data
        pot_slice = jnp.ones(shape, dtype=jnp.complex64)
        beam = jnp.ones(shape + (num_modes,), dtype=jnp.complex64)
    ...
    @chex.all_variants()
    @parameterized.parameters(
        {"shape": (100, 100), "num_modes": 3},
        {"shape": (200, 200), "num_modes": 5},
    )
    def test_cbed_single_slice_multi_beam_deterministic(self, shape, num_modes):
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape, dtype=jnp.complex64)
        beam = jax.random.normal(key, shape + (num_modes,), dtype=jnp.complex64)
        variant_cbed_single_slice_multi_beam = self.variant(
        ...
        result1 = variant_cbed_single_slice_multi_beam(pot_slice, beam)
        result2 = variant_cbed_single_slice_multi_beam(pot_slice, beam)
    ...
    def test_cbed_single_slice_multi_beam_single_mode(self, shape):
        num_modes = 1
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape, dtype=jnp.complex64)
        beam = jax.random.normal(key, shape + (num_modes,), dtype=jnp.complex64)
        ...
        result_multi = variant_cbed_single_slice_multi_beam(pot_slice, beam)
        result_single = variant_cbed_single_slice_single_beam(pot_slice, beam)

        # Compare with single beam calculation

        chex.assert_trees_all_close(result_multi, result_single)
...
class test_cbed_multi_slice_multi_beam(chex.TestCase):
    ...
    def test_cbed_multi_slice_multi_beam_shape_and_type(
        self, shape, num_slices, num_modes, slice_thickness, voltage_kV, calib_ang
    ):
        # Generate random complex input data
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape + (num_slices,), dtype=jnp.complex64)
        beam = jax.random.normal(key, shape + (num_modes,), dtype=jnp.complex64)

        # Get the variant of the function to test
        variant_cbed_multi_slice_multi_beam = self.variant(cbed_multi_slice_multi_beam)

        # Run the function
        result = variant_cbed_multi_slice_multi_beam(
            pot_slice, beam, slice_thickness, voltage_kV, calib_ang
        )
    ...
    def test_cbed_multi_slice_multi_beam_values(
        self, shape, num_slices, num_modes, slice_thickness, voltage_kV, calib_ang
    ):
        # Generate simple input data
        pot_slice = jnp.ones(shape + (num_slices,), dtype=jnp.complex64)
        beam = jnp.ones(shape + (num_modes,), dtype=jnp.complex64)
    ...
    def test_cbed_multi_slice_multi_beam_deterministic(
        self, shape, num_slices, num_modes, slice_thickness, voltage_kV, calib_ang
    ):
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape + (num_slices,), dtype=jnp.complex64)
        ...
        result1 = variant_cbed_multi_slice_multi_beam(
            pot_slice, beam, slice_thickness, voltage_kV, calib_ang
        )
        result2 = variant_cbed_multi_slice_multi_beam(
            pot_slice, beam, slice_thickness, voltage_kV, calib_ang
        )
    ...
    def test_cbed_multi_slice_multi_beam_single_mode(
        self, shape, num_slices, slice_thickness, voltage_kV, calib_ang
    ):
        num_modes = 1
        key = jax.random.PRNGKey(0)
        pot_slice = jax.random.normal(key, shape + (num_slices,), dtype=jnp.complex64)
        ...
        variant_cbed_multi_slice_multi_beam = self.variant(cbed_multi_slice_multi_beam)
        result_multi = variant_cbed_multi_slice_multi_beam(
            pot_slice, beam, slice_thickness, voltage_kV, calib_ang
        )

        # Compare with manual calculation for single mode
        slice_transmission = propagation_func(
            shape, slice_thickness, voltage_kV, calib_ang
        )
        real_space_convolve = jnp.ones(shape, dtype=jnp.complex64)
        for i in range(num_slices):
            this_slice = jnp.multiply(pot_slice[:, :, i], beam[:, :, 0])
            propagated_slice = jnp.fft.ifft2(
                jnp.multiply(jnp.fft.fft2(this_slice), slice_transmission)
            )
            real_space_convolve = jnp.multiply(real_space_convolve, propagated_slice)
        result_single = jnp.square(
            jnp.abs(jnp.fft.fftshift(jnp.fft.fft2(real_space_convolve)))
        )

        chex.assert_trees_all_close(result_multi, result_single, atol=1e-5)
