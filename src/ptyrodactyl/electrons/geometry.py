...
@jaxtyped(typechecker=beartype)
def rotmatrix_axis(axis: Float[Array, "3"], theta: scalar_float) -> Float[Array, "3 3"]:
    """
    Description
    -----------
    ...
    - Build rotation matrix using Rodrigues formula:
        - The formula is: R = I*cos(θ) + (1-cos(θ))*n⊗n + sin(θ)*[n]×
...
def rotate_structure(
    ...
    - Apply primary rotation to coordinates:
        - Multiply position vectors by transpose of rotation matrix: coords @ R.T
    ...
    - Rotate unit cell:
        - Apply same rotation to lattice vectors: cell @ R.T
    ...
    rotated_coords: Float[Array, "N 4"] = jnp.hstack((coords[:, 0:1], rotated_coords))
