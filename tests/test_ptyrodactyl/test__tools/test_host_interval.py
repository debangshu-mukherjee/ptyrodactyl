...
import ptyrodactyl._tools.host_interval as host_interval
...
def test_vc_shared_seams_are_identical_to_their_owner() -> None:
    """Keep every cross-module VC seam identical to its private owner."""
    for name in _SHARED_SEAM_NAMES:
        aggregate_seam = getattr(internal_tools, name)
        assert aggregate_seam is getattr(host_interval, name)
        assert getattr(vc_certificate, name) is aggregate_seam
    assert internal_tools.RationalInterval is host_interval.RationalInterval
    assert (
        internal_tools.normalized_sinc_integer_ratio
        is host_interval.normalized_sinc_integer_ratio
    )
...
def test_symbolic_quadrant_roots_remain_exact() -> None:
    ...
    observed = tuple(
        internal_tools.rational_turn_exponential(Fraction(turn, 4))
        for turn in range(4)
    )
...
def test_rectangle_arithmetic_and_pairwise_order_remain_exact() -> None:
    ...
    product = internal_tools.complex_rectangle_multiply(left, right)
    terms = (
    ...
    assert internal_tools.pairwise_rectangle_sum(terms) == (
        Fraction(4),
        Fraction(7),
        Fraction(-1),
        Fraction(2),
...
def test_normalized_sinc_symbolic_zeros_evenness_and_unwrapped_modes() -> None:
    ...
    assert internal_tools.normalized_sinc_integer_ratio(0, 5) == (
        Fraction(1),
        Fraction(1),
    )
    for mode in (5, -10, 5 * 10**100):
        assert internal_tools.normalized_sinc_integer_ratio(mode, 5) == (
            Fraction(0),
            Fraction(0),
        )

    for mode in (1, 6, 10**80 + 1):
        positive = internal_tools.normalized_sinc_integer_ratio(mode, 5)
        negative = internal_tools.normalized_sinc_integer_ratio(-mode, 5)
        assert negative == positive

    base = internal_tools.normalized_sinc_integer_ratio(1, 5)
    repetitions = 10**40
    huge_mode = 1 + 2 * 5 * repetitions
    huge = internal_tools.normalized_sinc_integer_ratio(huge_mode, 5)
    assert base[0] > 0
    assert huge == (base[0] / huge_mode, base[1] / huge_mode)

    negative_lobe = internal_tools.normalized_sinc_integer_ratio(6, 5)
    assert negative_lobe[1] < 0

    with pytest.raises(ValueError, match="count must be positive"):
        internal_tools.normalized_sinc_integer_ratio(1, 0)
...
def test_rational_sqrt_bounds_contain_every_scale(value: Fraction) -> None:
    ...
    assert internal_tools.sqrt_fraction_upper(value) == upper
...
def test_outward_binary64_and_verified_sqrt_regressions() -> None:
    ...
    lower = internal_tools.fraction_lower_float(one_third)
    upper = internal_tools.fraction_upper_float(one_third)
    ...
    assert internal_tools.fraction_lower_float(below_subnormal) == 0.0
    assert internal_tools.fraction_upper_float(below_subnormal) == tiny

    exact_root = internal_tools.sqrt_fraction_upper(Fraction(9, 16))
    irrational_root_upper = internal_tools.sqrt_fraction_upper(Fraction(2))
...
def test_point_to_rectangle_error_remains_exact() -> None:
    ...
    error = internal_tools.coefficient_error_fraction(
